...
<div id="sidebar" class="span3">
	...
	<ul id="sideManu" class="nav nav-tabs nav-stacked">
		...
		<li class="subMenu"><a>{{ $section['name'] }}</a>
			...
			<ul>
				<li><a href="{{ url($category['url']) }}"><i class="fa fa-chevron-right" aria-hidden="true"></i><strong>{{ $category['category_name'] }}</strong></a></li>
				@foreach($category['subcategories'] as $subcategory)
				<li><a href="{{ url($subcategory['url']) }}"><i class="fa fa-chevron-right" aria-hidden="true"></i>{{ $subcategory['category_name'] }}</a></li>
				@endforeach
			</ul>
			@endforeach
		</li>
